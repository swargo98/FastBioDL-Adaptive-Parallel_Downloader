...
"$PYTHON_BIN" -u benchmark_max_jobs_gridsearch_new_expanse.py \
    "${ACCESSIONS[@]}" \
    --repeats "$REPEATS" \
    --threads "$THREADS" \
    --work-root "$WORK_ROOT" \
...
cp "$REPO_DIR/run_benchmark_max_jobs_gridsearch_expanse_new_2.sh" "$LUSTRE_OUT/" 2>/dev/null || true
cp "$REPO_DIR/benchmark_max_jobs_gridsearch_new_expanse.py" "$LUSTRE_OUT/" 2>/dev/null || true
