...
GRID_PAIRS: List[Tuple[int, int]] = [(c, p) for c in (1, 2, 3, 4, 5, 6, 7, 8) for p in (1, 2, 3, 4, 5, 6, 7, 8)]
...
def benchmark_one_accession(
    ...
    for r in range(1, repeats + 1):
        ...
        try:
            seq_inputs = prepare_inputs(downloaded_sra, sra_dir, accession, n_copies=12)
    ...
    for max_c, max_p in GRID_PAIRS:
        ...
        for r in range(1, repeats + 1):
            ...
            pigz_dir = runs_pigz_root / run_id
            try:
                run_inputs = prepare_inputs(downloaded_sra, sra_dir, accession, n_copies=12)
