...
# --outdir flag).
#
#   PRODUCTION : root_dir = "<local_scratch>/fastbiodl/staging/"
...
class FileMover:
    ...
    def begin(self):
        # IMPORTANT: start worker processes BEFORE starting any feeder/reporter
        # threads. Forking a multithreaded Python process is unsafe and can
    ...
    def stop(self, timeout: float = 7200.0):
        ...
        while move_complete.value < transfer_complete.value or transfer_done.value == 0:
            progress = (move_complete.value, transfer_complete.value)
            if progress != last_progress:
                logger.info(f"[FileMover] Progress {move_complete.value}/{transfer_complete.value}, transfer_done={transfer_done.value}")
