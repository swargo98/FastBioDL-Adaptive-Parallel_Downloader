#!/bin/bash
# Run the multi-accession, multi-repeat benchmark on a Fabric (FABRIC testbed) node.
# Storage layout: sra_out=NVMe, fastq_out=NVMe, pigz_out=HDD.
#
# Usage:
#   ./run_benchmark_max_jobs_gridsearch_fabric.sh ACC1 ACC2 ACC3
#
...
#   NVME_MOUNT      NVMe mount point (default /mnt/nvme0)
#   HDD_MOUNT       HDD  mount point (default /mnt/hdd0)
...
#
# Adjust NVME_MOUNT and HDD_MOUNT to match the actual mount points on the
# Fabric slice. The defaults here are placeholders and will fail fast if those
# paths do not exist.
...
cp "$REPO_DIR/run_benchmark_max_jobs_gridsearch_fabric.sh" "$RESULTS_OUT/" 2>/dev/null || true
