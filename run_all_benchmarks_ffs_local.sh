#!/usr/bin/env bash
# Run the four end-to-end SRA benchmarks on a local lab server.
#
...
#
# Usage:
#   ./run_all_benchmarks_ffs_local.sh
#
...
#   HDD_MOUNT      slow-tier root           (default /mnt/storage)
#   RESULTS_ROOT   where final logs/results are aggregated (default $REPO_DIR/benchmark_results)
...
# --- aiohttp sanity check (required by fastbiodl_upgrade.py) ----------------
...
SRA_OUT_DIR="$NVME_BASE/sra"        # fast tier
FASTQ_OUT_DIR="$NVME_BASE/fastq"    # fast tier
PIGZ_OUT_DIR="$HDD_BASE/pigz"       # slow tier (final destination)
RESULTS_OUT="${RESULTS_ROOT:-$HDD_BASE/results}"
WORK_ROOT="$NVME_BASE/work"         # fastbiodl tmpfs/scratch root (fast tier)

mkdir -p "$SRA_OUT_DIR" "$FASTQ_OUT_DIR" "$PIGZ_OUT_DIR" "$RESULTS_OUT" "$WORK_ROOT"

# fastbiodl_upgrade.py picks fast-tier work from LOCAL_SCRATCH (storage_config.py)
...
# Each function takes a single argument: the accession list file path.
...
# --- Cleanup helper between iterations -------------------------------------
to_human() {
    local bytes="$1"
