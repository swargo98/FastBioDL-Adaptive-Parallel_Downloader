#!/bin/bash
# Run the storage-tier characterization benchmark on a local lab server.
# Mirrors run_benchmark_max_jobs_gridsearch_local.sh.
# Tiers: NVMe and HDD; cross-tier pair NVMe -> HDD is the move-stage
# workload on the lab server.
#
# Usage:
#   ./run_disk_io_benchmark_local.sh
#
# Optional environment overrides:
#   PYTHON_BIN      Python interpreter                      (default python3)
#   NVME_MOUNT      NVMe mount point                        (default /)
#   HDD_MOUNT       HDD  mount point                        (default /mnt/storage)
...
JSON_OUT="$RESULTS_OUT/disk_io_benchmark_local.json"
...
cp "$REPO_DIR/run_disk_io_benchmark_local.sh" "$RESULTS_OUT/" 2>/dev/null || true
